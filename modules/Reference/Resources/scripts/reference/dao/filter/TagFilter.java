...
public class TagFilter implements IFilter<Tag> {
    ...
    public Predicate collectPredicate(Root<Tag> root, CriteriaBuilder cb, Predicate condition) {
        ...
        if (category != null && !category.isEmpty()) {
            if (condition == null) {
                condition = cb.or(cb.equal(root.get("category"), ""), cb.like(cb.lower(root.get("category")), category));
            } else {
                condition = cb.or(cb.equal(root.get("category"), ""), cb.like(cb.lower(root.get("category")), category), condition);
            }
        }
