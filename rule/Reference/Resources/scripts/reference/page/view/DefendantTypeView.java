...
public class DefendantTypeView extends _DoPage {
	...
	public void doGET(_Session session, _WebFormData formData) {
		...
		if (user.getId() == SuperUser.ID || user.getRoles().contains("reference_admin")) {
			...
			_Action newDocAction = new _Action(getLocalizedWord("new_", session.getLang()), "", "new_defendant_type");
			newDocAction.setURL("Provider?id=defendanttype-form");
			actionBar.addAction(newDocAction);
			actionBar.addAction(new _Action(getLocalizedWord("del_document", session.getLang()), "", _ActionType.DELETE_DOCUMENT));
			addContent(actionBar);
		}
		addContent(getViewPage(new DefendantTypeDAO(session), formData));
	}
